"""Poll MongoDB for unprocessed direct-summary jobs.
...
def _worker_id() -> str:
    return f"{socket.gethostname()}-sonnet-direct-{time.time_ns()}"
...
def main() -> None:
    ...
    if not settings.credentials_ok:
        raise SystemExit("ANTHROPIC_API_KEY is not set in sonet_direct_summary/.env")

    worker_id = _worker_id()
    max_jobs = max(1, settings.worker_max_jobs)
    poll = max(0.5, settings.worker_poll_seconds)
    logger.info(
        "Direct-summary worker %s started (max_jobs=%s poll=%ss model=%s chunk_pages=%s)",
        worker_id,
        max_jobs,
        poll,
