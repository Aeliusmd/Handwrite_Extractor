"""Poll MongoDB for unprocessed jobs and run the existing extract pipeline.
...
def main() -> None:
    ...
    if not settings.credentials_ok:
        raise SystemExit("Google credentials are not configured.")

    worker_id = _worker_id()
    max_jobs = max(1, settings.worker_max_jobs)
    poll = max(0.5, settings.worker_poll_seconds)
    logger.info(
        "Worker %s started (max_jobs=%s poll=%ss docai=%s gemini=%s)",
        worker_id,
        max_jobs,
        poll,
        settings.docai_max_concurrent,
