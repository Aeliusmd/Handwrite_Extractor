...
def build_chunks(pdf_path: str, output_dir: Path) -> list[Chunk]:
    """Split into page windows that fit Anthropic PDF + extraction output limits.

    Hard caps: 100 pages (visual PDF quality) and ~20 MB raw (32 MB request after base64).
    Default window is smaller (CHUNK_PAGES) so per-page JSON is not truncated.
    """
...
def split_chunk(source_pdf: str, chunk: Chunk, output_dir: Path) -> list[Chunk]:
    """Bisect a chunk when Sonnet truncates or the JSON is incomplete."""
